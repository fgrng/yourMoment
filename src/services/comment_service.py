...
def validate_comment(
    ...
    """
    Validate a generated comment against quality and requirement criteria.

    Standalone function — no service dependencies, callable from tasks.

    Args:
        comment_content: Generated comment text (must already include AI prefix)
        min_length: Minimum allowed length of content after prefix
        max_length: Maximum allowed length of content after prefix
        enable_content_validation: Whether to run repetition/placeholder checks

    Returns:
        Dictionary with keys: is_valid, has_ai_prefix, errors, content_length
    """
    errors = []
...
class CommentService:
    ...
    async def _generate_with_provider(
        ...
        """
        try:
            # Generate comment using refactored LLM service
            # llm_service.generate_completion handles DTO conversion, structured output, and error mapping
            async with asyncio.timeout(self.config.generation_timeout):
                gen_result = await self.llm_service.generate_completion(
            ...
            response = CommentResponse(
                ...
                token_count=gen_result.total_tokens,
    ...
    def _ensure_german_prefix(self, comment_content: str) -> str:
        ...
        """
        return AIComment.apply_ai_prefix(comment_content)

    def _validate_comment(self, comment_content: str, request: CommentRequest) -> Dict[str, Any]:
        """Validate generated comment. Delegates to module-level validate_comment."""
        return validate_comment(
            comment_content,
            min_length=self.config.min_comment_length,
    ...
    async def _rate_limit_provider(self, provider_id: uuid.UUID):
        ...
        async with self._generation_lock:
            ...
            self._last_generation_time[provider_id] = asyncio.get_running_loop().time()
