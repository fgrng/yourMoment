...
def _attach_retry_context(task, *, retries=0, max_retries=3, retry_callback=None):
    task.request = SimpleNamespace(id="fixture-task-id", retries=retries)
...
async def test_post_single_comment_wrapper_reverts_claim_before_retry(
    ...
    task = _bind_task_sessions(CommentPostingTask(), db_engine)
    _attach_retry_context(task, retries=0, max_retries=3, retry_callback=_raise_retry)

    with pytest.raises(RetryScheduled):
        comment_posting.post_comment_for_article.run.__func__(task, str(scenario["ai_comment"].id))
...
async def test_post_single_comment_wrapper_marks_failed_after_retry_budget_is_exhausted(
    ...
    task = _bind_task_sessions(CommentPostingTask(), db_engine)
    _attach_retry_context(task, retries=3, max_retries=3)
    mark_failed_statuses: list[str] = []
    original_mark_comment_failed = task._mark_comment_failed

    async def probe_mark_comment_failed(ai_comment_id, error_msg, expected_status="posted"):
    ...
    result = comment_posting.post_comment_for_article.run.__func__(task, str(scenario["ai_comment"].id))
...
async def test_post_single_comment_wrapper_marks_auth_failures_failed_without_retry(
    ...
    task = _bind_task_sessions(CommentPostingTask(), db_engine)
    _attach_retry_context(task, retries=0, max_retries=3)
    result = comment_posting.post_comment_for_article.run.__func__(task, str(scenario["ai_comment"].id))
...
async def test_post_single_comment_concurrent_claim_allows_only_one_http_post(
    ...
    assert second_result["reason"] in {"already_claimed", "already_posted"}
...
async def test_post_single_comment_wrapper_uses_posted_fallback_when_claim_revert_is_stuck(
    db_session,
    db_engine,
):
    scenario = await build_scenario("generated_not_posted", db_session)
    scenario["ai_comment"].status = "posted"
    await db_session.commit()
    ...
    assert result["status"] == "failed"
    assert task.retry_calls == []
    assert mark_failed_statuses == ["generated", "posted"]
    await db_session.refresh(scenario["ai_comment"])
    assert scenario["ai_comment"].status == "failed"
