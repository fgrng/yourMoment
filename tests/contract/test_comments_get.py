...
async def test_get_comment_detail_success():
    ...
    async with AsyncClient(transport=transport, base_url="http://test") as client:
        ...
        assert payload["reasoning_content"] == "Reasoning trace"
        assert payload["generation_tokens"] == 321
        assert payload["generation_time_ms"] == 1500
        assert payload["article_task_id"] == 42
        assert "prompt_template_id" in payload
        assert "llm_provider_id" in payload
...
@pytest.mark.contract
@pytest.mark.asyncio
async def test_list_comments_includes_extended_fields():
    app, db_session = await create_test_app()
    email, password = await create_test_user(app, db_session)
    user = await _get_user(db_session, email)
    comment = await _seed_comment(db_session, user.id)
    ...
    async with AsyncClient(transport=transport, base_url="http://test") as client:
        ...
        assert response.status_code == 200
        payload = response.json()
        item = next(item for item in payload["items"] if item["id"] == str(comment.id))

        assert item["reasoning_content"] == "Reasoning trace"
        assert item["generation_tokens"] == 321
        assert item["generation_time_ms"] == 1500
        assert item["article_task_id"] == 42
        assert "article_edited_at" in item
        assert "prompt_template_id" in item
